...
public class BinaryHeap<E extends Comparable<E>> {
	...
	private BinaryHeap(BinaryHeap<E> heap) {
		...
		for(int i=0;i<this.size();i++){
			this.hashMap.put(this.array.get(i),i);
		}
	}
	...
	private void hashmapMAJ(E value, int index){
		this.hashMap.put(value,index);
	}

	public Map<E, Integer> getHashMap() {
		return hashMap;
	}
	...
	public boolean update(E x) {
		boolean insert_ou_non=false;
		Integer j=this.hashMap.get(x); //on ne se sert pas de getElementPosition qui utilise lui-même un get qui renverra null si element
		//pas present dans hashmap mais plantage car getE_P renvoie un int (supporte pas le champs null)
		if (j==null) {
			insert(x);
			j=this.hashMap.get(x);
			insert_ou_non=true;
		}
			this.percolateUp(j);
			this.percolateDown(j);
		return insert_ou_non;
	...
	private void percolateUp(int index) {
		...
		for( ; index > 0 && x.compareTo(this.array.get(index_parent(index)) ) < 0; index = index_parent(index) ) {
			...
			this.hashmapMAJ(moving_val, index);
		}
		...
		if (ileft < this.currentSize) {
			...
			if (!hasRight || left.compareTo(right) < 0) {
				...
				if (left.compareTo(current) < 0) {
					...
					this.hashmapMAJ(left, index);
					this.arraySet(ileft, current) ;
					this.hashmapMAJ(current, index);
	...
	public E deleteMin( ) {  
		...
		this.hashmapMAJ(lastItem, 0);
		this.percolateDown( 0 );
		this.hashMap.remove(minItem); //IMPORTANT !!!!!
		return minItem;
	}
