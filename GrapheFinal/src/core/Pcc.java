...
public class Pcc extends Algo {
	...
	//protected HashMap< Noeud,Label_Dij> mapLabel; //fait correspondre un noeud ������ un label
	...
	public int choix;//en temps (choix=1),  en distance (choix=0)
	protected String Affichage;//Afficher ou non le d������roulement de l'algo



	public Pcc(Graphe gr, PrintStream sortie, int depart, Readarg readarg) {
		...
		if ( (origine<=0) || (origine>graphe.getReseau().length) ){
			System.out.println(" Le numero de sommet d'origine saisi n'appartient pas au graphe");
	...
	public void affichercout(){
		...
		if(choix==0){ //distnode].getListeDeRoutes()ance
	...
	public void Dijkstra() {
		...
		boolean bool=false;// covoiturage ou non=> 1 vers tous ou 1 vers 1
		if (destination<0||(destination>graphe.getReseau().length))
			bool=true;
		else
				...
				if(!(labsucc.getmarquage())){
					...
					if (choix==1) { //choix==1 temps
						...
						if (destination==-1 && Covoiturage.nature){ //pieton
							...
							//################### A 4km/h l'auto va toujours chercher le pieton donc on test à 30km/h (vélo)
